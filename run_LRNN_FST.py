...
def run_trial(SEED, hid_feature_size=-1, hid_seq_size=-1, transformer_nhead=-1, hid_rnn_size=-1, blocks=-1, dropout=-1,  use_fst=-1, use_transformer=-1):
    ...
    for epoch in tqdm(range(50)):
        ...
        pickle.dump(results, open(f'gridsearch_results/result_LRNN_FST_{SEED}.pkl', 'wb'))
        
seed_n = 0

for n_blocks in [1,2,3]:
    for hid_feature_size in [128,256,512]:
        for hid_seq_size in [128,256,512]:
            for hid_rnn_size in [128,256,512]:
                run_trial(seed_n, hid_feature_size=hid_feature_size, hid_seq_size=hid_seq_size, transformer_nhead=1, hid_rnn_size=hid_rnn_size, blocks=n_blocks, dropout=0.2,  use_fst=True, use_transformer=False)
                seed_n += 1
                run_trial(seed_n, hid_feature_size=hid_feature_size, hid_seq_size=hid_seq_size, transformer_nhead=1, hid_rnn_size=hid_rnn_size, blocks=n_blocks, dropout=0.2,  use_fst=False, use_transformer=False)
                seed_n += 1